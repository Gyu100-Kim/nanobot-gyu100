...
## 비유로 먼저 이해하기 — 요리 재료 목록 읽는 법

요리를 배울 때 레시피의 **재료 목록**부터 보면 요리 전체가 짐작되듯이, 프로그램도
"어떤 라이브러리(남이 만들어 둔 부품)를 쓰는지"를 보면 성격이 보입니다. 그 재료 목록이
`pyproject.toml`이라는 파일입니다.

세 가지만 알면 이 문서 전체가 읽힙니다.

1. **필수 재료 vs 선택 재료** — 밥솥(핵심 기능)에 꼭 필요한 재료는 항상 설치하고,
   "텔레그램 연동" 같은 특별 반찬 재료는 **원하는 사람만** 설치하게 나눠 놨습니다.
   이 "선택 재료" 방식을 extras(선택 의존성, lazy deps)라고 부릅니다.
   덕분에 기본 설치가 가볍고 빠릅니다.
2. **재료의 버전 표기** — `anthropic>=0.45.0,<1.0.0`은 "0.45 이상, 1.0 미만 버전만 허용"이라는
   뜻입니다. 유통기한 표시처럼, 너무 오래된 것도 너무 새것(호환이 깨질 수 있는 것)도 막는
   안전장치입니다.
3. **재료가 없을 때의 안내** — 선택 재료가 없는 기능을 켜려고 하면
   `optional_features.py`라는 코드가 "이 재료가 필요해요, 설치해 드릴까요?"라고
   감지하고 안내(또는 자동 설치)합니다.

**꼭 가져가야 할 것 3가지**

1. `pyproject.toml`의 `dependencies` = 항상 설치되는 필수 부품, `optional-dependencies` = 골라 쓰는 부품.
2. 버전에 하한(`>=`)과 상한(`<`)을 함께 두는 것은 "안정성과 최신성 사이의 절충"이다.
3. `nanobot/optional_features.py`가 선택 부품의 설치 여부 확인·설치·기능 켜기를 담당한다.
