...
## 실전 예제로 차근차근 따라가기 — 서류 가방 하나가 싸이는 순서

한 턴에서 LLM에게 건네질 "가방"이 실제로 어떤 순서로 채워지는지 따라가 봅니다.

**1단계 — 지침서부터.** `build_system_prompt()`가 조각을 차례로 쌓습니다:
정체성(`identity.md` 렌더링) → 워크스페이스의 `AGENTS.md`/`SOUL.md`/`USER.md` →
도구 사용 규약(`tool_contract.md`) → 장기 기억(`MEMORY.md`, 단 기본 템플릿 그대로면 생략) →
항상 켜진 스킬 본문 → 나머지 스킬의 **목차만** → 최근 history 발췌. 마지막에 전부
`---` 구분선으로 이어 하나의 긴 system 메시지가 됩니다.

**2단계 — 대화 기록.** [06](06_state_and_persistence.md)의 `get_history()`가 만든
"정리된 최근 대화"가 system 메시지 뒤에 이어 붙습니다.

**3단계 — 지금 받은 말 + 상황표.** 여러분의 새 메시지가 user 메시지로 붙는데,
그 **뒤에** 현재 시각·채널·채팅방 ID 같은 런타임 정보가 태그로 감싸여 따라붙습니다.
왜 뒤일까요? 시각은 매 턴 바뀌므로 앞에 두면 프롬프트 캐시가 매번 깨지기 때문입니다 —
바뀌지 않는 부분을 앞에, 바뀌는 부분을 뒤에 두는 것이 캐시 절약의 요령입니다.

**4단계 — 무게 검사.** 완성된 가방을 보내기 직전 `ContextGovernor.prepare_for_model`이
검사합니다: 깨진 메시지 정리 → 도구 호출과 결과의 짝 맞추기 → 도구 결과 길이 제한 →
그래도 예산(context_window − 출력 여유분)을 넘으면 오래된 히스토리부터 잘라내기.
이 모든 손질은 **보낼 사본에만** 가해지고, 디스크의 원본 대화는 그대로입니다.

**막간 — 한가할 때의 사서.** 세션이 한동안 조용하면 `AutoCompact`가 배경에서 미리
요약을 만들어 둡니다. 다음에 말을 걸면 COMPACT 단계에서 그 요약(`pending_summary`)을
바로 꺼내 쓰므로, 오랜만의 첫 응답도 느려지지 않습니다.
