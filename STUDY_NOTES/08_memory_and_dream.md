...
## 비유로 먼저 이해하기 — 일기장, 수면 정리, 그리고 요리 비법 노트

사람의 기억이 "오늘 있었던 일(단기)"과 "정리된 지식(장기)"으로 나뉘듯, nanobot의 기억도
두 층입니다.

- **일기장(`history.jsonl`)** — 나눈 대화를 시간 순서대로 빠짐없이 적어 두는 곳.
- **정리 노트(`MEMORY.md`)** — 일기에서 중요한 것만 골라 정리한 장기 기억.

그런데 누가 일기를 정리 노트로 옮길까요? 바로 **Dream(꿈)**입니다. 사람이 자는 동안 뇌가
그날 기억을 정리하듯, nanobot도 주기적으로 AI에게 "최근 일기를 읽고 정리 노트를 고쳐 써 줘"
라고 시킵니다. 이때 성격 파일(SOUL.md)과 사용자 정보(USER.md)도 함께 다듬습니다.
어디까지 정리했는지는 책갈피(커서)로 표시해 같은 페이지를 두 번 정리하지 않습니다.

**스킬(Skill)**은 별개의 개념으로, "요리 비법 노트"입니다. 특정 작업(예: 요약하기,
GitHub 쓰기)의 요령을 적은 SKILL.md 마크다운 문서이며, 코드가 아니라 **글로 가르치는
능력 확장**입니다. 평소에는 목차만 AI에게 보여 주고, 그 작업을 할 때만 본문을 읽게 해서
서류 가방(컨텍스트)을 아낍니다.

**꼭 가져가야 할 것 3가지**

1. 기억은 2층 구조: 전부 적는 `history.jsonl`(일기) + 정리된 `MEMORY.md`(장기 기억).
2. Dream = 주기적으로 AI가 일기를 읽고 장기 기억·성격 파일을 스스로 고쳐 쓰는 "수면 정리".
3. Skill = 코드가 아닌 마크다운 문서로 능력을 가르치는 방식, 필요할 때만 본문을 로드한다.
...
## 실전 예제로 차근차근 따라가기 — 기억 하나가 장기 기억이 되기까지

여러분이 대화 중에 "참, 나 다음 주에 부산으로 이사 가"라고 말했다고 합시다.
이 정보가 nanobot의 장기 기억이 되는 과정입니다.

**1단계 — 일기에 적힌다.** 턴이 끝나면 이 대화의 요지가 `append_history(...)`를 통해
`memory/history.jsonl`에 한 줄 추가되고, 자동 증가 커서(예: 137번)를 받습니다.
아직은 "정리 안 된 일기"일 뿐입니다.

**2단계 — 밤이 온다(Dream 트리거).** cron이 `"dream"` 작업 시각을 알리면
`on_cron_job`이 에이전트 루프를 거치지 않고 직접 Dream을 실행합니다.
`build_dream_prompt()`가 `.dream_cursor`(예: 120)를 확인하고, 121~137번의 미처리
일기와 **현재 MEMORY.md/SOUL.md/USER.md의 실제 내용**을 하나의 프롬프트로 묶습니다.

**3단계 — 제한된 손으로 정리.** LLM은 이 프롬프트를 읽고 "사용자가 부산으로 이사"라는
사실을 `MEMORY.md`에 적어 넣습니다. 이때 쓸 수 있는 도구는 파일 읽기/편집/쓰기/패치
네 개뿐입니다 — 셸이나 웹은 아예 주지 않습니다(최소 권한).

**4단계 — 말이 아니라 diff로 검증.** Dream이 끝나면 `dream_content_diff()`가 git으로
파일이 **실제로** 바뀌었는지 확인합니다. 바뀌었으면 커서를 137로 전진시키고 변경을
커밋하며, LLM이 "정리했어요"라고 말만 하고 파일을 안 바꿨다면 커서를 유지해
다음 Dream이 같은 일기를 다시 처리하게 합니다.

**5단계 — 다음 대화에서 되살아난다.** 이후 어떤 채널에서 대화를 시작해도
`build_system_prompt()`가 `MEMORY.md`를 시스템 프롬프트에 넣으므로, AI는
"사용자는 부산에 산다"를 처음부터 알고 대화를 시작합니다. 세션(단기)과 달리
이 기억은 채널과 세션을 넘어 영구히 유지됩니다.
