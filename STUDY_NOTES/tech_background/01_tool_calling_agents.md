...
"서울 날씨 알려줘"라는 질문 하나로 이 문서 전체를 복습해 봅시다.

1. 런타임이 LLM에게 대화와 함께 `get_weather(city: string)` 같은 **도구 목록(JSON Schema)**을 보여 줍니다.
2. LLM은 답을 지어내는 대신 `{"name": "get_weather", "arguments": {"city": "Seoul"}}`라는
   **구조화된 요청서**를 씁니다 — 이것이 tool call입니다.
3. 런타임이 실제 날씨 API를 호출하고, 결과를 그 요청서의 **ID에 짝을 맞춰** 대화에 붙입니다.
4. LLM이 결과를 읽고 "서울은 지금 맑고 3도입니다"라고 최종 답을 씁니다 — 루프 종료.
5. 도구가 실패했다면? 오류 메시지도 결과로 돌려주고, LLM이 읽고 다른 방법을 시도합니다.

이 다섯 줄이 ReAct(추론↔행동 반복)의 아이디어이자, nanobot `runner.py`의
`for iteration in range(...)` 루프가 하는 일의 전부입니다.

### 직접 확인해 볼 질문

1. LLM이 도구를 "직접 실행"하지 못하는데도 에이전트가 행동할 수 있는 이유는? (역할 분리)
2. 도구 결과에 `tool_call_id`가 왜 반드시 필요할까? (병렬 호출과 짝 맞추기)
3. 도구 실행이 실패했을 때 예외로 죽이지 않고 오류 문자열을 돌려주는 이유는?

다음으로 읽으면 좋은 문서: 도구가 많아질수록 대화가 길어지는 문제 →
[02_context_compression.md](02_context_compression.md), 이 계약을 표준화한 프로토콜 →
[04_mcp.md](04_mcp.md).
