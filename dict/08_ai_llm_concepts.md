...
### LLM
**클래스:** [Concept](00_content_classes.md#concept) · **한글:** 대형 언어 모델 (Large Language Model)
...
- **상위 개념(더 특수):** [Transformer](#transformer), [Tool Calling](#tool-calling)
- **관련 용어:** [Prompt](#prompt), [Sampling](04_providers_and_llm.md#sampling)

### Transformer
**클래스:** [Concept](00_content_classes.md#concept) · **한글:** 트랜스포머

논문 *Attention Is All You Need*(Vaswani et al., 2017)가 제안한 신경망 구조로, 현대
[LLM](#llm)의 사실상 유일한 토대. 핵심은 [Self-Attention](#self-attention) — 문장 안의 모든
단어 쌍의 관련도를 병렬로 계산해, 순서대로만 읽던 이전 구조(RNN)의 한계를 넘었습니다.
(nanobot은 모델을 API로 쓰므로 트랜스포머를 직접 구현하지 않습니다.)

- **하위 개념(더 일반):** [LLM](#llm)
- **상위 개념(더 특수):** [Self-Attention](#self-attention)

### Self-Attention
**클래스:** [Concept](00_content_classes.md#concept) · **한글:** 셀프 어텐션

[Transformer](#transformer)의 핵심 연산 — 입력의 각 [Token](#token)이 다른 모든 토큰을 "얼마나
주목할지"를 학습된 가중치로 계산합니다. 계산량이 토큰 수의 제곱에 비례하기 때문에
[Context Window](#context-window)에 상한이 생기고, 그래서
[Context Compression](#context-compression)이 필요해집니다 — 이 사전의 여러 용어가 결국 이 제곱
비용에서 파생됩니다.

- **하위 개념(더 일반):** [Transformer](#transformer)
- **관련 용어:** [Lost in the Middle](#lost-in-the-middle)

### Prompt
**클래스:** [Concept](00_content_classes.md#concept) · **한글:** 프롬프트
...
- **상위 개념(더 특수):** [System Prompt](03_memory_context_session.md#system-prompt),
...
- **상위 개념(더 특수):** [Few-shot Learning](#few-shot-learning),
  [Chain-of-Thought](#chain-of-thought)
...
- **상위 개념(더 특수):** [BPE](#bpe), [Tokenizer](#tokenizer)
...
- **하위 개념(더 일반):** [Token](#token)
- **상위 개념(더 특수):** [tiktoken](#tiktoken), [BPE](#bpe)

### BPE
**클래스:** [Concept](00_content_classes.md#concept) · **한글:** 바이트 쌍 인코딩 (Byte Pair Encoding)

자주 함께 등장하는 문자 쌍을 반복 병합해 어휘를 만드는 [Tokenizer](#tokenizer) 알고리즘
(Sennrich et al., 2016에서 신경망 번역에 도입). 희귀 단어도 조각으로 분해해 표현할 수 있어
"모르는 단어" 문제가 사라집니다.

- **하위 개념(더 일반):** [Tokenizer](#tokenizer), [Token](#token)
- **상위 개념(더 특수):** [tiktoken](#tiktoken)

### tiktoken
**클래스:** [Technology](00_content_classes.md#technology) · **PyPI:** `tiktoken`
...
- **하위 개념(더 일반):** [BPE](#bpe), [Tokenizer](#tokenizer)
...
- **상위 개념(더 특수):** [RAG](#rag), [Tool Calling](#tool-calling)

### In-Context Learning
**클래스:** [Concept](00_content_classes.md#concept) · **한글:** 문맥 내 학습
...
- **상위 개념(더 특수):** [Few-shot Learning](#few-shot-learning), [Zero-shot](#zero-shot)
- **관련 용어:** [Fine-tuning](#fine-tuning)

### Few-shot Learning
**클래스:** [Concept](00_content_classes.md#concept) · **한글:** 퓨샷 학습
...
- **하위 개념(더 일반):** [In-Context Learning](#in-context-learning),
...
### Fine-tuning
**클래스:** [Concept](00_content_classes.md#concept) · **한글:** 파인튜닝(미세 조정)
...
- **상위 개념(더 특수):** [PEFT](#peft)
- **관련 용어:** [In-Context Learning](#in-context-learning)

### PEFT
**클래스:** [Concept](00_content_classes.md#concept) · **한글:** 파라미터 효율적 파인튜닝 (Parameter-Efficient Fine-Tuning)
...
- **하위 개념(더 일반):** [Fine-tuning](#fine-tuning)
- **상위 개념(더 특수):** [LoRA](#lora)

### LoRA
**클래스:** [Concept](00_content_classes.md#concept) · **한글:** 로라 (Low-Rank Adaptation)
...
- **하위 개념(더 일반):** [PEFT](#peft)

### RAG
**클래스:** [Concept](00_content_classes.md#concept) · **한글:** 검색 증강 생성 (Retrieval-Augmented Generation)
...
- **하위 개념(더 일반):** [Grounding](#grounding), [In-Context Learning](#in-context-learning)
...
- **상위 개념(더 특수):** [Vector Database](#vector-database)
- **관련 용어:** [RAG](#rag)
...
- **하위 개념(더 일반):** [Embedding](#embedding)
...
- **상위 개념(더 특수):** [AgentLoop](01_core_architecture.md#agentloop),
  [AgentRunner](01_core_architecture.md#agentrunner), [ReAct](#react)
- **하위 개념(더 일반):** [Agent](01_core_architecture.md#agent)

### Tool Calling
**클래스:** [Concept](00_content_classes.md#concept) · **한글:** 도구 호출 (function calling)
...
- **하위 개념(더 일반):** [LLM](#llm), [Grounding](#grounding)
- **상위 개념(더 특수):** [Tool](01_core_architecture.md#tool), [MCP](#mcp)
...
- **상위 개념(더 특수):** [Tool Schema](02_tools_and_skills.md#tool-schema)
- **관련 용어:** [Pydantic](09_dev_stack.md#pydantic)

### ReAct
**클래스:** [Research](00_content_classes.md#research)
...
- **하위 개념(더 일반):** [Agent Loop (concept)](#agent-loop-concept)
- **관련 용어:** [Chain-of-Thought](#chain-of-thought), [Tool Calling](#tool-calling)

### Chain-of-Thought
**클래스:** [Research](00_content_classes.md#research) · **한글:** 사고 사슬 (CoT)
...
- **하위 개념(더 일반):** [Prompt Engineering](#prompt-engineering), [Prompt](#prompt)
- **관련 용어:** [ReAct](#react)
...
- **상위 개념(더 특수):** [Summarization](#summarization), [Sliding Window](#sliding-window),
...
- **하위 개념(더 일반):** [Context Compression](#context-compression)
- **상위 개념(더 특수):** [Consolidation](03_memory_context_session.md#consolidation)
...
### Hierarchical Memory
**클래스:** [Concept](00_content_classes.md#concept) · **한글:** 계층적 메모리
...
- **상위 개념(더 특수):** [Memory](03_memory_context_session.md#memory)
- **관련 용어:** [Dream](03_memory_context_session.md#dream)

### Reflection
**클래스:** [Concept](00_content_classes.md#concept) · **한글:** 반성(자기 성찰)
...
- **상위 개념(더 특수):** [Dream](03_memory_context_session.md#dream)
...
- **상위 개념(더 특수):** [Skill](01_core_architecture.md#skill), [Voyager](#voyager),
...
- **하위 개념(더 일반):** [Skill Library](#skill-library)
...
- **하위 개념(더 일반):** [Skill Library](#skill-library)
- **상위 개념(더 특수):** [SKILL.md](02_tools_and_skills.md#skillmd)

### MCP
**클래스:** [Protocol](00_content_classes.md#protocol) · **한글:** 모델 컨텍스트 프로토콜 (Model Context Protocol)
...
- **하위 개념(더 일반):** [Tool Calling](#tool-calling), [JSON-RPC](#json-rpc)
- **상위 개념(더 특수):** [MCPToolWrapper](02_tools_and_skills.md#mcptoolwrapper),
...
### SSE
**클래스:** [Protocol](00_content_classes.md#protocol) · **한글:** 서버 전송 이벤트 (Server-Sent Events)
...
- **하위 개념(더 일반):** [HTTP](05_channels_gateway_ui.md#http)
...
- **하위 개념(더 일반):** [MCP](#mcp), [HTTP](05_channels_gateway_ui.md#http)

### LSP
**클래스:** [Protocol](00_content_classes.md#protocol) · **한글:** 언어 서버 프로토콜 (Language Server Protocol)

에디터와 언어 분석기를 분리한 Microsoft의 표준 — "M개 에디터 × N개 언어"를 M+N으로 줄인
선례로, [MCP](#mcp) 설계의 직접적 영감입니다. [JSON-RPC](#json-rpc) 기반이라는 점도 같습니다.

- **하위 개념(더 일반):** [JSON-RPC](#json-rpc)
- **관련 용어:** [MCP](#mcp)
...
- **상위 개념(더 특수):** [Model Preset](01_core_architecture.md#model-preset),
