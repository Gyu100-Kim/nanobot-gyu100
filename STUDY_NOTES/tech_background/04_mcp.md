...
## 비유로 먼저 이해하기 — 도구 세계의 USB 규격

예전에는 휴대폰마다 충전 단자가 달라서 기기마다 다른 케이블이 필요했습니다. USB-C라는
표준이 나오면서 하나의 케이블로 통일됐죠. **MCP(Model Context Protocol)는 "AI 도구
연결의 USB-C"**입니다.

- MCP가 없다면: 에이전트 앱마다 × 도구마다 연동 코드를 따로 짜야 합니다 (N×M 문제).
- MCP가 있다면: 도구 제공자는 "MCP 서버"를 한 번만 만들고, 어떤 에이전트든 "MCP
  클라이언트"로 꽂아 쓰면 됩니다 (N+M으로 감소).

구조는 세 역할입니다: **호스트**(에이전트 앱, 예: nanobot) — **클라이언트**(연결 담당) —
**서버**(도구/데이터 제공자). 바닥의 통신은 JSON-RPC라는 오래된(2005~) 단순 규약이고,
연결 방법은 두 가지입니다: 같은 컴퓨터 안이면 stdio(파이프), 원격이면 HTTP.

주의할 점: 외부 서버를 꽂는 만큼 **보안 위험도 함께 꽂힙니다**. 악의적 서버가 도구 설명에
몰래 지시문을 심는 공격(tool poisoning) 등이 알려져 있어, nanobot은 규격 위반/이상 응답에
방어적으로 구현되어 있습니다.

**꼭 가져가야 할 것 3가지**

1. MCP = 도구 연동의 표준 플러그 규격 (N×M 문제를 N+M으로 줄임).
2. 역할은 호스트/클라이언트/서버 3층, 바닥은 JSON-RPC, 연결은 stdio 또는 HTTP.
3. 외부 서버는 편리한 만큼 위험하므로 방어적 구현과 신뢰 관리가 필수다.
...
사내 DB 조회 기능을 nanobot에 붙이고 싶은 개발자의 하루:

1. MCP가 없다면 — nanobot 코드를 고쳐 도구를 추가해야 하고, 다른 에이전트(Claude
   Desktop 등)에 또 붙이려면 같은 일을 반복해야 합니다.
2. MCP가 있으면 — DB 조회 기능을 **MCP 서버** 하나로 만들어 두면 끝. MCP를 말하는
   어떤 클라이언트(nanobot 포함)든 그 서버에 접속해 도구 목록을 받아 씁니다. USB처럼
   "한 번 규격에 맞춰 만들면 어디에나 꽂히는" 구조입니다.
3. 접속하면 클라이언트가 `tools/list`(JSON-RPC)로 "무슨 도구 있어요?"를 묻고, 서버가
   이름·설명·JSON Schema를 돌려줍니다.
4. LLM이 그 도구를 호출하면 클라이언트가 `tools/call`로 서버에 전달하고 결과를 받아
   대화에 붙입니다 — 이후는 일반 tool calling과 동일합니다.
5. nanobot에서는 `agent/tools/mcp.py`가 이 클라이언트 역할을 하며, 설정에 서버 주소만
   적으면 외부 MCP 서버의 도구들이 자동으로 도구 목록에 합류합니다.

### 직접 확인해 볼 질문

1. MCP가 해결하는 "M×N 문제"란 무엇인가?
2. MCP 도구와 nanobot 내장 도구는 LLM의 눈에 어떻게 보일까? (동일하게 — 왜?)
3. 원격 MCP 서버를 붙일 때 생기는 보안 고려사항은?

다음으로: 도구 계약의 기초 → [01_tool_calling_agents.md](01_tool_calling_agents.md),
본편 [05_tools.md](../05_tools.md).
