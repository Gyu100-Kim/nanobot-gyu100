...
## 실전 예제로 차근차근 따라가기 — "메모.txt 요약해 줘" 한 문장의 여정

지금까지 배운 것을 하나의 구체적인 장면으로 묶어 봅니다. 여러분이 Telegram으로
nanobot에게 **"메모.txt 요약해 줘"**라고 보냈다고 합시다. 이 문장이 코드 속을 어떻게
여행하는지 역을 하나씩 따라가 보겠습니다.

**1단계 — 접수.** Telegram 채널이 메시지를 받아 `InboundMessage(channel="telegram",
chat_id="12345", content="메모.txt 요약해 줘")`를 만들어 `bus.publish_inbound(...)`로
접수함에 넣습니다. 이 시점에 에이전트는 아직 아무것도 모릅니다 — 그저 큐에 요청서가
한 장 쌓였을 뿐입니다.

**2단계 — 팀장이 꺼낸다.** `AgentLoop.run()`(loop.py L937의 `while` 루프)이
`consume_inbound()`로 요청서를 꺼냅니다. 이 세션(`telegram:12345`)이 지금 처리 중이
아니므로, L1015에서 새 비동기 태스크로 `_dispatch(msg)`를 시작합니다.
(만약 같은 방에서 연달아 두 번째 메시지를 보냈다면? L985-L1012의 주입(injection) 경로로
진행 중인 턴에 끼워 넣어져, 대화가 꼬이지 않습니다.)

**3단계 — 체크리스트 시작.** `_process_message`가 상태머신을 돌립니다.
- **RESTORE**: `sessions.get_or_create("telegram:12345")`로 이 방의 대화 공책을 폅니다.
- **COMPACT**: 공책이 길면 미리 준비된 요약이 있는지 확인합니다.
- **COMMAND**: "메모.txt 요약해 줘"는 `/`로 시작하지 않으므로 명령이 아님 → `"dispatch"`
  이벤트로 BUILD로 넘어갑니다.
- **BUILD**: `get_history(...)`로 최근 대화를 꺼내고, 시스템 프롬프트·기억·스킬 목차와 합쳐
  `initial_messages`(AI에게 보여 줄 서류 뭉치)를 조립합니다. 여러분의 문장은 이 뭉치의
  맨 끝에 user 메시지로 붙습니다.

**4단계 — 실무자의 반복.** RUN 단계에서 `AgentRunner._run_core`의 `for iteration in
range(...)` 루프가 시작됩니다.
- 1회차: 서류 뭉치를 LLM에 보냅니다. LLM은 "메모.txt 내용을 봐야겠네"라고 판단하고
  `read_file(path="메모.txt")`라는 도구 호출을 돌려줍니다(`should_execute_tools`가 참).
  runner가 도구를 실행하고, 파일 내용이 `{"role": "tool", ...}` 메시지로 대화에 붙습니다.
- 2회차: 파일 내용까지 본 LLM이 이번에는 도구 호출 없이 요약문을 씁니다.
  도구 호출이 없으므로 반복이 끝나고, 요약문이 `final_content`가 됩니다.

**5단계 — 기록하고 답장.** SAVE 단계가 이번 턴에 새로 생긴 메시지들(여러분의 질문,
도구 호출, 도구 결과, 최종 요약)을 공책에 적고 `sessions.save(...)`로 원자적 저장합니다.
RESPOND 단계가 요약문을 `OutboundMessage`로 포장해 발송함에 넣고, Telegram 채널이
그것을 꺼내 여러분의 채팅방에 전송합니다. 여러분 눈에는 "요약해 줘" 몇 초 뒤에
요약문이 도착한 것으로만 보이지만, 그 사이에 이 모든 역을 거친 것입니다.

이 여정을 그림 한 장으로 요약한 것이 바로 아래의 시퀀스 다이어그램입니다.
